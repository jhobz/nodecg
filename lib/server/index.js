...
module.exports.start = function () {
	...
	const envPort = process.env.PORT ? process.env.PORT : config.port;

	log.trace(`Attempting to listen on ${config.host}:${envPort}`);
	server.on('error', err => {
		switch (err.code) {
			case 'EADDRINUSE':
				...
				log.error(`[server.js] Listen ${config.host}:${envPort} in use, is NodeCG already running? NodeCG will now exit.`);
				break;
			default:
	...
	server.listen({
		host: config.host,
		port: process.env.NODECG_TEST ? undefined : envPort
	}, () => {
		if (process.env.NODECG_TEST) {
